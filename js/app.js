...
(() => {
  ...
  function renderTabs() {
    ...
    addTab.type = "button";
    addTab.setAttribute("aria-label", "Tambah folder baru");
    addTab.innerHTML = `<span class="tab__symbol">+</span><span class="tab__name">Folder baru</span>`;
  ...
  // ---------------------------------------------------------------
  // MODALS: tambah kosakata / tambah folder / export
  // ---------------------------------------------------------------
  function openModal(type) {
    if (type === "term") {
      ...
      const cat = getCurrentCategory();
      el("#bulk-modal-cat-name").textContent = cat ? cat.name : "";
      el("#bulk-form").dataset.categoryId = currentCategoryId;
      el("#bulk-textarea").value = "";
      el("#bulk-form-error").textContent = "";
      showModal("#bulk-modal");
      el("#bulk-textarea").focus();
    } else if (type === "export") {
  ...
  function bindGlobalEvents() {
    ...
    el("#bulk-add-btn").addEventListener("click", () => openModal("bulk"));
    ...
    el("#bulk-form").addEventListener("submit", (e) => {
      e.preventDefault();
      const form = e.target;
      const categoryId = form.dataset.categoryId;
      const raw = el("#bulk-textarea").value;
      const entries = GeodesiStorage.parseBulkText(raw);
      if (!entries.length) {
        el("#bulk-form-error").textContent = "Belum ada istilah yang terbaca — cek pemisah \";\"-nya.";
        return;
      }
      const count = GeodesiStorage.addTermsBulk(categoryId, entries);
      closeModal(el("#bulk-modal"));
      refreshData();
      toast(`${count} kosakata ditambahkan.`);
    });
